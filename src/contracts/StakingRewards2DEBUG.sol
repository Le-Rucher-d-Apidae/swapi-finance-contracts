...
import { StakingRewardsEvents } from "./StakingRewards2Events.sol";
...
contract StakingRewards2DEBUG is ReentrancyGuard, Ownable(msg.sender), Pausable, IStakingRewards2Errors, StakingRewardsEvents {
    ...
    function lastTimeRewardApplicable() public /* view */ returns (uint256) {
        emit DBG( "lastTimeRewardApplicable", Math.min(block.timestamp, periodFinish ));

        return Math.min(block.timestamp, periodFinish);
    }

    function rewardPerToken() public /* view */ returns (uint256) {
        emit DBG( "rewardPerToken:isVariableRewardRate",isVariableRewardRate);
        if (isVariableRewardRate) {
            emit DBG( "rewardPerToken:",constantRewardRatePerTokenStored);
            return constantRewardRatePerTokenStored;
        }
        if (_totalSupply == 0) {
            emit DBG( "rewardPerToken:",rewardPerTokenStored);
            return rewardPerTokenStored;
        }

        emit DBG( "rewardPerToken:rewardPerTokenStored",rewardPerTokenStored);
        emit DBG( "rewardPerToken:lastTimeRewardApplicable()",lastTimeRewardApplicable());
        emit DBG( "rewardPerToken:lastUpdateTime",lastUpdateTime);
        emit DBG( "rewardPerToken:rewardRate",rewardRate);
        emit DBG( "rewardPerToken:_totalSupply",_totalSupply);
        emit DBG( "rewardPerToken:result",rewardPerTokenStored
    ...
    function earned(address account) public /* view */ returns (uint256) {
        emit DBG("earned:account", account);
        emit DBG("earned:isVariableRewardRate", isVariableRewardRate);

        if (isVariableRewardRate) {
            emit DBG("earned:isVariableRewardRate");
        ...
        emit DBG("earned:_balances[account]", _balances[account]);
        emit DBG("earned:constantRewardRatePerTokenStored", constantRewardRatePerTokenStored);
        emit DBG("earned:lastTimeRewardApplicable()", lastTimeRewardApplicable());
        emit DBG("earned:userLastUpdateTime[account]", userLastUpdateTime[account]);
        emit DBG("earned:rewards[account]", rewards[account]);

        emit DBG("earned:lastTimeRewardApplicable() - userLastUpdateTime[account]", lastTimeRewardApplicable() - userLastUpdateTime[account]);
        emit DBG("earned:_balances[account] * constantRewardRatePerTokenStored", _balances[account] * constantRewardRatePerTokenStored);
        emit DBG("earned:return", _balances[account] * constantRewardRatePerTokenStored
                * (lastTimeRewardApplicable() - userLastUpdateTime[account]) / ONE_TOKEN + rewards[account]);
    ...
    function getRewardForDuration() external /* view */ returns (uint256) {
        emit DBG("getRewardForDuration");
        if (isVariableRewardRate) {
            // Current MAX possible reward for duration
            emit DBG("getRewardForDuration", constantRewardRatePerTokenStored * variableRewardMaxTotalSupply * rewardsDuration);
            return constantRewardRatePerTokenStored * variableRewardMaxTotalSupply * rewardsDuration;
        }
        emit DBG("getRewardForDuration", rewardRate * rewardsDuration);
    ...
    {
        emit DBG("stakeWithPermit", 0);
        if (amount == 0) revert StakeZero();
        emit DBG("stakeWithPermit:amount", 0);
        _totalSupply = _totalSupply + amount;
        emit DBG("stakeWithPermit:_totalSupply", 0);
        emit DBG("stakeWithPermit:isVariableRewardRate", isVariableRewardRate);
        if (isVariableRewardRate) {
            emit DBG("stakeWithPermit:isVariableRewardRate TRUE", isVariableRewardRate);
            if (_totalSupply > variableRewardMaxTotalSupply) {
                emit DBG("stakeWithPermit:isVariableRewardRate variableRewardMaxTotalSupply", variableRewardMaxTotalSupply);
                revert StakeTotalSupplyExceedsAllowedMax(_totalSupply, variableRewardMaxTotalSupply);
            }
        }
        emit DBG("stakeWithPermit:isVariableRewardRate _balances[msg.sender]", _balances[msg.sender]);
        _balances[msg.sender] = _balances[msg.sender] + amount;
        emit DBG("stakeWithPermit:isVariableRewardRate _balances[msg.sender]", _balances[msg.sender]);

        emit DBG("stakeWithPermit:isVariableRewardRate PERMIT");
        // permit
        IUniswapV2ERC20(address(stakingToken)).permit(msg.sender, address(this), amount, deadline, v, r, s);
        emit DBG("stakeWithPermit:isVariableRewardRate PERMIT", true);

        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        emit DBG("stakeWithPermit:isVariableRewardRate safeTransferFrom", amount);
        if (isVariableRewardRate) {
            emit DBG("stakeWithPermit:Update variable reward rate");
            // Update variable reward rate
            emit DBG("stakeWithPermit:constantRewardRatePerTokenStored", constantRewardRatePerTokenStored);
            emit DBG("stakeWithPermit:_totalSupply", _totalSupply);
            variableRewardRate = constantRewardRatePerTokenStored * _totalSupply;
            emit DBG("stakeWithPermit:variableRewardRate", variableRewardRate);
        }
        emit DBG("stakeWithPermit:Staked");
        emit Staked(msg.sender, amount);
    }
    ...
    modifier updateReward(address account) {
        ...
        emit DBG("updateReward:account", account);
        emit DBG("updateReward:isVariableRewardRate", isVariableRewardRate);
        if (isVariableRewardRate) {
            emit DBG("isVariableRewardRate:");
            // console.log( "updateReward: isVariableRewardRate" );
            // Update variable reward rate
            emit DBG("updateReward:isVariableRewardRate", isVariableRewardRate);
            rewardPerTokenStored = constantRewardRatePerTokenStored;

            emit DBG("updateReward:rewardPerTokenStored", rewardPerTokenStored);

            if (account != address(0)) {
                // console.log( "updateReward: earned(account) = ", earned(account) );
                rewards[account] = earned(account);
                emit DBG("updateReward:earned(account)", rewards[account]);
                // console.log( "updateReward: rewardPerTokenStored = ", rewardPerTokenStored );
                userLastUpdateTime[account] = lastTimeRewardApplicable();
                emit DBG("updateReward:earned(account)", userLastUpdateTime[account]);
            }
            ...
            emit DBG("updateReward:lastUpdateTime", lastUpdateTime);
            // console.log( "updateReward: ! isVariableRewardRate" );
            // console.log( "updateReward: rewardPerTokenStored = ", rewardPerTokenStored );
            ...
            if (account != address(0)) {
                ...
                emit DBG("updateReward:userRewardPerTokenPaid[account]", userRewardPerTokenPaid[account]);
            }
        }
        _;
    }
    ...
    event DBG(string msg);
    event DBG(string msg, bool val);
    event DBG(string msg, address val);
    event DBG(string msg, uint256 val);
